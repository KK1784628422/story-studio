...
/**
 * Build a shell-free invocation. Prefer the repo-local agent-browser js entry
 * (node_modules/agent-browser/bin/agent-browser.js, same absolute path the server
 * uses) — agent 的 Bash/子进程环境 PATH 常不含 node_modules/.bin，按命令名 spawn 会
 * ENOENT；用本地 js 入口 + process.execPath 可彻底绕开 PATH 依赖。回退：Windows 解析
 * `.cmd` shim、POSIX 用原生二进制。
 */
function localAgentBrowserJs() {
  try {
    // scripts/ → 技能目录 → vendor → packages → story-studio（仓库根）
    const repoRoot = path.resolve(__dirname, "..", "..", "..", "..", "..");
    const js = path.join(repoRoot, "node_modules", "agent-browser", "bin", "agent-browser.js");
...
function buildAgentBrowserInvocation(port, args, platform = process.platform) {
  ...
  const localJs = localAgentBrowserJs();
  if (localJs) {
    if (platform === "win32") {
      // 桌面应用无控制台环境：node 包装器（bin/agent-browser.js）内部 spawn 原生 exe 时
      // 固定 windowsHide:false —— 每次调用都会弹一个黑色 cmd 窗口。直接 exec 原生二进制
      // （调用侧 execFileSync 已带 windowsHide:true）即可彻底消除。
      const native = path.join(path.dirname(localJs), `agent-browser-win32-${process.arch}.exe`);
      if (fs.existsSync(native)) return { file: native, args: argv };
    }
    return { file: process.execPath, args: [localJs, ...argv] };
  }
  if (platform !== "win32") {
    return { file: "agent-browser", args: argv };
  }
