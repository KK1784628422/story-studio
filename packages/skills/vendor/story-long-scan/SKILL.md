...
> 🖥️ **桌面应用（Electron）优先路径**：若存在 `{项目根}/.local/embedded-cdp-port`（内置浏览器在线），番茄/七猫/晋江/刺猬猫直接走**内置浏览器**采集——采集脚本内部自动读取内置端口，页面在「Agent浏览器」面板可视化；**禁止运行 setup-cdp-chrome.js**（它只探测 9222 调试 Chrome，会误判未就绪并诱导 kill 用户的常规 Chrome）。仅纯浏览器模式才按 browser-cdp 的 9222 流程走。
