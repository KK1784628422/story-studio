...
const USAGE = `Usage: node check-degeneration.js [--check] [--json] [--fail-on=blocking|all] [--skip-meta-leak] <file...>
...
--skip-meta-leak: 关闭工程词泄漏检查。细纲/大纲等规划文件用——字数目标/情节点/钩子/下一章
是其本职字段（细纲模板本身就含这些词），在正文里才判违规；此类文件只查复读/截断/占位。
...
const options = { json: false, files: [], failOn: 'all', skipMetaLeak: false };

for (let i = 2; i < process.argv.length; i += 1) {
  const arg = process.argv[i];
  if (arg === '--check') {
    // Accepted for symmetry with the other detectors; detection is always check-only.
  } else if (arg === '--skip-meta-leak') {
    options.skipMetaLeak = true;
  } else if (arg === '--json') {
    options.json = true;
...
for (const file of options.files) {
  ...
  const findings = scanDocument(input, options.skipMetaLeak).map((finding) => ({ file, ...finding }));
...
function scanDocument(input, skipMetaLeak) {
  ...
  // 细纲/大纲等规划文件的工程词是本职字段（字数目标/情节点/钩子…），只对正文判违规；
  // --skip-meta-leak 关闭该项后，此类文件仍保留复读/截断/占位三类退化检查。
  if (!skipMetaLeak) findings.push(...findMetaLeak(content));
