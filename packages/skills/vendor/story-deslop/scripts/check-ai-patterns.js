...
Detect high-risk AI-flavor prose patterns that need human rewrite:
  ...
  - 俏皮重复谚语 (不X，白不X 即「不吃白不吃」族，口语复读绕弯，改为直白因果)
  - 绕口令复读 (说X也X，说Y也Y 式空转对比) + 模糊否定堆叠 (X不得，Y不得) + 徒劳自问自答 (还能干什么？什么都干不了。) + 极短应和句中句号 (「稳健。明日…」≤3字短语被句号截续话)

Each finding carries severity: blocking by default for generation/deslop cleanup (not-is-comparison / voice-contrast / negation-parade / reverse-not-is / trailer-ending / trailer-summary / reduplicative-colloquial). This is a local style/readability gate, not an AIGC detector score; functional human text can be marked for review instead of hard-edited for a detector.
...
// 俏皮重复谚语（实战漏网 F）：「不V，白不V」即「不吃白不吃」族。
// 口语惯用「不V，白不V」= 错过了才可惜，纯句法复读读感绕弯，读者第一眼未必秒懂
// （「不偷听，白不偷听」被人要求改成「错过才是傻子」）。做 blocking，命中即改成
// 直白因果或具体动作；「不吃白不吃」这类确为大众谚语的可由人工豁免。
const REDUPLICATIVE_COLLOQUIAL_PATTERN = /不([\u3400-\u9fff]{1,3})[，,]?\s*白不\1/g;

// 绕口令复读（实战新增 1）：「说好过也好过，说难熬也难熬」说X也X，说Y也Y 式空转对比，太啰嗦。
const VERBOSE_REFLEXIVE_PATTERN = /说([\u3400-\u9fff]{1,3})也\1[，,]\s*说([\u3400-\u9fff]{1,3})也\2/g;
// 模糊否定堆叠（实战新增 2）：「动不得，说不得」X不得，Y不得，语义含糊（“不得”不能/不许两可），改具体。
const DEBUDE_ECHO_PATTERN = /([\u3400-\u9fff]{1,2})不得[，,]\s*([\u3400-\u9fff]{1,2})不得/g;

// 俏皮重复谚语（实战漏网 F）：引号外叙述逐处 blocking，位置与摘录取自原文。
function findReduplicativeColloquial(proseLines) {
  const findings = [];
  ...
  for (const { text, lineNo } of proseLines) {
    ...
    REDUPLICATIVE_COLLOQUIAL_PATTERN.lastIndex = 0;
    let match;
    while ((match = REDUPLICATIVE_COLLOQUIAL_PATTERN.exec(masked)) !== null) {
      findings.push({
        line: lineNo,
        column: match.index + 1,
        type: 'reduplicative-colloquial',
        severity: 'blocking',
        message: '俏皮重复谚语：「不X，白不X」口语复读读感绕弯、读者未必秒懂；改成直白因果或具体动作（如「错过才是傻子」），或删掉复读直接写后果。',
...
// 绕口令复读（实战新增 1）：引号外叙述逐处 blocking，「说X也X，说Y也Y」式空转对比。
function findVerboseReflexive(proseLines) {
  const findings = [];
  ...
  for (const { text, lineNo } of proseLines) {
    ...
    if (!trimmed || isDivider(trimmed) || isStructural(trimmed)) continue;
    const masked = maskQuoted(text);
    VERBOSE_REFLEXIVE_PATTERN.lastIndex = 0;
    let match;
    while ((match = VERBOSE_REFLEXIVE_PATTERN.exec(masked)) !== null) {
      findings.push({
        line: lineNo,
        column: match.index + 1,
        type: 'verbose-reflexive',
        severity: 'blocking',
        message: '绕口令复读：「说X也X，说Y也Y」式空转对比太啰嗦；删掉对仗铺排，直接写一个具体的观感或事实。',
        excerpt: compact(text.slice(match.index, match.index + match[0].length)),
      });
    }
  }
...
// 模糊否定堆叠（实战新增 2）：引号外叙述逐处 blocking，「X不得，Y不得」语义含糊。
function findDebudeEcho(proseLines) {
  const findings = [];
  ...
  for (const { text, lineNo } of proseLines) {
    ...
    if (!trimmed || isDivider(trimmed) || isStructural(trimmed)) continue;
    const masked = maskQuoted(text);
    DEBUDE_ECHO_PATTERN.lastIndex = 0;
    let match;
    while ((match = DEBUDE_ECHO_PATTERN.exec(masked)) !== null) {
      findings.push({
        line: lineNo,
        column: match.index + 1,
        type: 'debude-echo',
        severity: 'blocking',
        message: '模糊否定堆叠：「X不得，Y不得」的「不得」语义含糊（是不能还是不许两可）；改成具体表达，如「动也动不了，说又没法说」。',
        excerpt: compact(text.slice(match.index, match.index + match[0].length)),
      });
    }
  }
...
// 徒劳自问自答（实战新增 3）：跨段「他还能干什么？」+「什么都干不了。」式空转絮语。
// 问句与扁平否定答句相邻（同段或隔空行，≤3 行），即报 blocking；只匹配反问+否定收场的组合，
// 不误伤真正的悬念问句。做 blocking，命中即删掉或合并成一句具体动作/后果。
function findRhetoricalQaEcho(proseLines) {
  const findings = [];
  const question = /^[^，。！？!?\n]{0,10}还能[\u3400-\u9fff]{1,8}[？?]\s*$/;
  const flatAnswer = /^什么[都也][\u3400-\u9fff]{1,8}不了[。！]?\s*$/;
  let pending = null;
  ...
  for (const { text, lineNo } of proseLines) {
    ...
    if (question.test(masked)) {
      pending = { lineNo, text: trimmed };
      continue;
    }
    if (flatAnswer.test(masked) && pending && lineNo - pending.lineNo <= 3) {
      findings.push({
        line: pending.lineNo,
        column: 1,
        type: 'rhetorical-qa-echo',
        severity: 'blocking',
        message: '徒劳自问自答：「还能干什么？什么都干不了」是空转絮语、没给新信息；删掉或合并成一句具体的动作/后果，交代他到底干了什么。',
        excerpt: compact(`${pending.text} ${trimmed}`),
      });
      pending = null;
    }
  }
...
// 对话引号内句中句号（实战新增 4）：直接引语里出现非末位的「。」——即被句号从中截成两段
// 再接话（如「昨日断案，今日又说服了赵公明那莽汉。做得尚可。」「稳健。明日…」）。
// 按用户要求：宽口径，一句引语里任何非末位句号都报；只查直接引语（“”/「」），排除【】系统面板。
// 注意：多句完整台词（「你且先退下。此事容我三思。」）也会命中，属预期，由人工判断是否改写。
function findMidSpeechPeriod(proseLines) {
  const findings = [];

  for (const { text, lineNo } of proseLines) {
    const trimmed = text.trim();
    if (!trimmed || isDivider(trimmed) || isStructural(trimmed)) continue;
    for (const [start, end] of quotedRanges(text)) {
      if (text[start] === '【') continue; // 系统面板/公告不是对话
      const innerEnd = end - 1;
      for (let p = start + 1; p < innerEnd; p += 1) {
        if (text[p] !== '。') continue;
        let hasAfter = false;
        for (let q = p + 1; q < innerEnd; q += 1) {
          if (!/\s/.test(text[q])) { hasAfter = true; break; }
        }
        if (!hasAfter) continue;
        findings.push({
          line: lineNo,
          column: p + 1,
          type: 'mid-speech-period',
          severity: 'blocking',
          message: '对话引号内句中句号：一句引语被「。」从中截成两段再接话；改成一气说完（续用逗号/感叹号），或拆成两句独立的引语。',
          excerpt: compact(text.slice(Math.max(0, p - 10), innerEnd + 2)),
        });
        break; // 同一引语只报第一处
      }
    }
...
function scanProsePatterns(proseLines) {
  ...
  findings.push(...findReduplicativeColloquial(proseLines));
  findings.push(...findVerboseReflexive(proseLines));
  findings.push(...findDebudeEcho(proseLines));
  findings.push(...findRhetoricalQaEcho(proseLines));
  findings.push(...findMidSpeechPeriod(proseLines));
