...
## ⭐ 桌面应用（Electron）优先——先确认内置浏览器是否在线

**若存在 `{项目根}/.local/embedded-cdp-port` 文件（桌面应用启动时由主进程写入）**，说明内置浏览器（WebContentsView）已在 **:port** 在线、登录态长期复用：

- **直接** `agent-browser --cdp {port} …`（或调用 browser_cdp 工具的 open/eval/click/login…），页面在「Agent浏览器」面板里可视化；
- **禁止运行 setup-cdp-chrome.js / 禁止探测 9222**——9222 只属于纯浏览器模式，探测会误判为 `needs-setup`，并诱导 kill 用户的常规 Chrome；
- 扫榜采集脚本会**自动读取内置端口**（内部 resolveScanPort），无需手工传参；
- 只有该文件**不存在**（纯浏览器/网页模式）才走下方 9222「启动流程」。
