...
const PLATFORM_CONFIG = {
  ...
  win32: (() => {
    // 浏览器内核选择：STORY_BROWSER_KIND=edge 用 Edge（跟随用户打开项目的浏览器，登录态/习惯一致）；
    // 默认 auto=探测 Edge→Chrome（Edge 优先，因多数用户用 Edge 打开项目）；=chrome 强制 Chrome。
    const KIND = /^(edge|chrome|brave)$/i.test(String(process.env.STORY_BROWSER_KIND || ""))
      ? String(process.env.STORY_BROWSER_KIND).toLowerCase()
      : null;
    const DEFS = {
      chrome: {
        exeName: "chrome.exe",
        ...
        profileDir: path.join(process.env.LOCALAPPDATA || path.join(os.homedir(), "AppData", "Local"), "Google", "Chrome", "User Data"),
      },
      edge: {
        exeName: "msedge.exe",
        paths: [
        ...
        profileDir: path.join(process.env.LOCALAPPDATA || path.join(os.homedir(), "AppData", "Local"), "Microsoft", "Edge", "User Data"),
      },
      brave: {
        exeName: "brave.exe",
        paths: [
        ...
        profileDir: path.join(process.env.LOCALAPPDATA || path.join(os.homedir(), "AppData", "Local"), "BraveSoftware", "Brave-Browser", "User Data"),
      },
    };
    // 选择实际使用的内核：显式指定 > Edge > Chrome > Brave（取第一个已安装）
    const order = KIND ? [KIND] : ["edge", "chrome", "brave"];
    let chosen = DEFS.chrome;
    for (const k of order) {
      const d = DEFS[k];
      if (d && d.paths.some((p) => p && fs.existsSync(p))) { chosen = d; break; }
    }
    return {
      chromePaths: chosen.paths,
      profileDir: chosen.profileDir,
      findChrome() {
      ...
      listChromePids() {
        try {
          const out = execSync(`tasklist /FI "IMAGENAME eq ${chosen.exeName}" /NH /FO CSV`, { encoding: "utf-8" }).trim();
          return out.split("\n").map((line) => {
            const m = line.match(new RegExp('"' + chosen.exeName.replace(".", "\\.") + '","(\\d+)"', "i"));
            return m ? parseInt(m[1], 10) : 0;
          }).filter((n) => n > 0);
        } catch { return []; }
      },
      killChrome() {
        try { execSync(`taskkill /F /IM ${chosen.exeName}`, { stdio: "ignore" }); } catch {}
      },
    };
  })(),
  linux: {
...
async function main() {
  ...
  const debugProfile = path.join(os.homedir(), "chrome-debug-profile" + (process.env.STORY_BROWSER_KIND ? `-${String(process.env.STORY_BROWSER_KIND).toLowerCase()}` : ""));
