...
def structural_type_errors(document: object) -> list[str]:
    """提交前置的轻量结构预扫：把「该是数组/对象/整型」的字段类型错误一次性聚合，
    而不是让深层校验只报第一个。返回字段级错误列表，供 dry-run 校验与 commit 预检共用。"""
    errors: list[str] = []
    if not isinstance(document, dict):
        return ["transaction must be a JSON object"]
    for field in ("schema_version", "mode", "chapter", "expected_state_revision"):
        if field not in document:
            continue
        value = document[field]
        if field == "mode":
            if value not in ("append", "revision"):
                errors.append(f"mode must be 'append' or 'revision' (got {value!r})")
    ...
    if not isinstance(document.get("delta"), dict):
        errors.append("delta must be a JSON object")
    else:
        for field in (
            "character_changes", "foreshadow_changes", "timeline_events", "constraints",
            "next_chapter_commitments", "retired_context_items", "retired_characters",
        ):
            value = document["delta"].get(field)
            if value is not None and not isinstance(value, list):
                errors.append(f"delta.{field} must be a JSON array (got {type(value).__name__})")
    if not isinstance(document.get("context"), dict):
        errors.append("context must be a JSON object")
    else:
        if document["context"].get("position") is not None and not isinstance(document["context"]["position"], dict):
            errors.append("context.position must be a JSON object")
        for field in ("long_term_constraints", "active_character_names", "continuity_risks"):
            value = document["context"].get(field)
            if value is not None and not isinstance(value, list):
                errors.append(f"context.{field} must be a JSON array (got {type(value).__name__})")
    if document.get("character_snapshots") is not None and not isinstance(document["character_snapshots"], dict):
        errors.append("character_snapshots must be a JSON object")
    return errors


def validate_transaction(project: Path, document: object) -> dict[str, Any]:
    """dry-run：跑完整校验但不落盘、不加锁。返回 {valid, errors}；结构类字段错误一次聚拢，
    深层首个语义错误最后补上，避免与 normalize_transaction 内部的结构预扫重复报告。"""
    ...
    try:
        normalize_transaction(project, state, document)
    except TrackingError as exc:
        message = str(exc)
        if message.startswith("transaction payload has structural errors"):
            error_messages.extend(
                line.lstrip().lstrip("- ").strip() for line in message.splitlines() if line.startswith("  - ")
            )
        else:
            error_messages.append(message)
    return {"valid": not error_messages, "errors": error_messages}


def normalize_transaction(project: Path, state: dict[str, Any], document: object) -> dict[str, Any]:
    structural = structural_type_errors(document)
    if structural:
        raise TrackingError(
            "transaction payload has structural errors:\n  - " + "\n  - ".join(structural)
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    for command in ("init", "commit", "validate"):
...
def main() -> int:
    ...
    try:
        if args.command == "init":
            ...
            result = validate_transaction(args.project, read_json(args.input))
            if not result["valid"]:
                emit(json.dumps(result, ensure_ascii=False), error=True)
                return 2
