...
- **设定/关系.md**：角色关系映射（参考 character-relations.md「四种关系类型」）。**必产文件，不可省略**：必须含「关系总览」Markdown 表格（`| 角色 A | 角色 B | 关系类型 | 情感倾向 | … |`），前两列为纯角色名且与 `设定/角色/{角色名}.md` 文件名一致——前端人物关系网按该表格逐行解析，格式不符图谱为空（硬性约束见 artifact-protocols.md）
