...
**机器可读格式约束（硬性）：**「关系总览」表格是本文件的核心，前端「人物关系网」可视化按它逐行解析生成节点和关系线，格式不符则图谱为空。必须遵守：
1. 表格必须存在，列顺序固定：`| 角色 A | 角色 B | 关系类型 | 情感倾向 | 当前状态 | 起始章节 | 变化节点 |`；
2. **前两列必须是纯角色名**（与 `设定/角色/{角色名}.md` 文件名完全一致，不加身份/修饰词/别名），每对角色一行；
3. 关系类型/情感倾向用短语（亲情/爱情/友情/敌对/师生/主从/利益 × 正面/负面/中性/复杂）；
4. 其余小节（关系演变/核心冲突关系）可自由发挥，但不要在表格之外重复罗列关系对。
